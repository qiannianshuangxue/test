...
如果auto_load_libs为False，那么外部函数将无法解析，每次调用它时，它都返回一个惟一的无约束符号值。  
```
#### hook 
```
...
obj.entry 对象入口点地址  
obj.min_addr, obj.max_addr 内存中最低地址和最高地址  
...
state.regs.rbp = state.regs.rsp rsp值赋给rbp  
...
val_to_bv方法用来转换成int类型   val_to_fp 用来转回float  
...
#### Simulation Managers   
```
simgr = proj.factory.simulation_manager(state)  <SimulationManager with 1 active> 输出活动数量  
simgr.active  
...
simgr.step()  
simgr.active [<SimState @ 0x400540>] ___libc_start_main plt表地址  
...
simgr.mp_authenticated mp_ 获取输入状态  
